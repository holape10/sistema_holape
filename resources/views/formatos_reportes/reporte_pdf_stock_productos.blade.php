...
<body  style="font-family:Helvética,sans-serif;font-size:8pt;">
	...
	<table style="width:100%;">
		<thead>
			...
			<tr >
				<th colspan="5" style="text-align:left;">{{$datos->direccion}}</th>
...
<table style="width:100%;" class="table-comprobantes">
				...
				<thead>
					...
					<tr>
						<th colspan="9"><center>STOCK DE PRODUCTOS {{Carbon::now()->format('d-m-Y')}} @if(!empty($datos)) - {{$datos->tipo_negocio}} @endif @if(!empty($datosalm)) - <br>{{$datosalm->descripcion}} @endif</center></th>
					</tr>
					<tr>
						<th colspan="9"><hr></th>
					</tr>
				...
				<tr>
					...
					<td>{{$pro->marca}}</td>
					<td>{{$pro->umenom}}</td>
					<td style="text-align:right;">{{$pro->stock}}</td>

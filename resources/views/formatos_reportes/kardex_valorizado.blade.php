...
.page-break {
			...
			<table id="dtHorizontalExample"  class="table table-responsive table-striped table-bordered table-sm">
							<thead>
								...
								<tr style="background:blue;">
									<th colspan="4"><center><font color="white"><strong>DOCUMENTO DE TRASLADO, COMPROBANTE DE PAGO, DOCUMENTO INTERNO O SIMILAR</strong></font></center></th>
          ...
          $saldo_anterior = ($calcular_saldo->Ingresos-($calcular_saldo->Egresos+$calcular_saldo->Anula)); 
								...
								<tr>
									   ...
									   <td>{{$c->tdocod}}</td>
										<td>{{$c->serie}}</td>
										<td>{{$c->numero}}</td>
										{{number_format($c->stock_inicial,'3','.',',')}}</td>

...
function Calcular(ele) {
  ...
  $(tr).each(function() {
    ...
    totitemgrav = $(this).find("td:eq(1) > input").val() * $(this).find("td:eq(3) > input").val();
 ...
 function agregarnota(button){
  ...
  /*if(contar>0){
      ...
      $('#grdet').append("<tr>"+
        ...
        "<td hidden='hidden' ><input readonly='readonly'  value='"+unidad+"' style='width:100px' name='unid[]'  class='form-control input-sm'></td>"+
        ...
        "<td hidden='hidden' ><input  type='number' step='any' min='0' class='form-control input-sm' name='desc[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='0.00' style='width:80px' ></td>"+
        "<td><input  type='text' class='form-control input-sm' name='itemtotal[]'  value='"+total+"' onkeyup='CalcularItem(this);' style='width:80px' ></td>"+
     ...
     <div class="col-lg-12"> 
        <div class="box">
          <div class="box-body">
            ...
            <div class="col-lg-3">
               <a href="/pedidos"><button type="button" class="btn-success btn btn-block btn-sm">NUEVO PEDIDO</button></a>
            </div>
                   ...
                   <table class="table table-hover" id="grdet">
                    ...
                    <tbody>
                      @foreach($detalle as $det)
                        <tr><td width='900px'><input type='text' class='form-control' name='pronom[]' value='{{$det->cdedes}}'></td><td> <input type='number' step='any' min='0' value='{{$det->cdecan}}' name='cant[]' onkeyup='Calcular(this);' onchange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'> </td><td hidden='hidden'><select style='width:100px' name='unid[]'  class='form-control input-sm'> @foreach($unidades as $und) @if($und->umecod == 'UNI') <option  selected='selected' value='{{$und->umecod}}'>{{$und->umenom}}</option> @else <option  value='{{$und->umecod}}'>{{$und->umenom}}</option> @endif @endforeach </select></td><td><input  type='number' step='any' min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='{{$det->cdepuni}}' style='width:80px' ></td><td hidden="hidden"><input  type="number" step="any" min="0" class="form-control input-sm" name="desc[]" onChange="Calcular(this);"  onkeyup="Calcular(this);" value="{{$det->desc_mon}}" style="width:80px" ></td><td><input type='text' class='form-control' name='itemtotal[]'  value='{{$det->cdevve}}' onkeyup='CalcularItem(this);' style='width:80px' ></td><td hidden='hidden'><input type='text' class='form-control' name='proid[]'  value='{{$det->IdProducto}}' readonly='readonly' ></td><td><button type='button' onClick='deleteRow(this);' class='btn btn-danger btn-sm remove'><span class='glyphicon glyphicon-minus'></span></button></td><td hidden=""><input type="hidden" value="{{$det->IdCpe_detalle}}" name="detalle[]"></td><td hidden="hidden" ><input  type='number' step='any'  class='form-control input-sm' name='propunref[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' min='{{$det->cdepuni}}' value='"+precio+"' style='width:80px' ></td></tr>
                      @endforeach
                    </tbody>
                  </table>
...
<div class="col-lg-6">
  <div class="col-lg-12">
        <div class="box">
         ...
         <div class="box-header" style="background-color:blue;">
              <font color="white" size="3"><center><strong>DATOS DEL PEDIDO {{$cabecera->numdoc}}</strong></center></font>
         </div>
         <div class="box-body">
             ...
             <div hidden="hidden" class="row">
              <div class="col-lg-4">
                <div class="form-group form-group-sm">
                  ...
                  <label >
                    <input type="radio" name="tdocod" id="pedido" value="16" checked="checked" > NV
                  </label>
    ...
    @endif
           ...
           <label >
            @if($cabecera->tdocod_fac =='03')
             <input type="radio" name="tdocod_1" id="boleta" value="03"  checked="checked"> BO
          @else
          ...
          @endif
           ...
           @if($cabecera->tdocod_fac =='01')
               <input type="radio" name="tdocod_1" id="factura" value="01" checked="checked"  > FA
          @else
          ...
          @else
               <input type="radio" name="tdocod_1" id="nota" value="13"    > NV
              ...
              <div class="form-group">
                ...
                <select class="form-control selectpicker input-sm" data-show-subtext="true" data-live-search="true" name="clicod" id="clicod" onchange="seleccionarcliente();">
                  ...
                  @foreach($clientes as $cliente)
                    ...
                    <option selected="selected" value="{{$cliente->clicod}}" data-documento="{{$cliente->tdicod}}" data-clinum="{{$cliente->clinum}}" data-direccion="{{$cliente->clidir}}" data-clinom="{{$cliente->clinom}}" data-correo="{{$cliente->clicor}}" data-vendedor="{{$cliente->vendedor}}" data-telefono="{{$cliente->telefono}}">{{$cliente->clinum}} - {{$cliente->clinom}}</option>
                       @else
                        <option value="{{$cliente->clicod}}" data-documento="{{$cliente->tdicod}}" data-clinum="{{$cliente->clinum}}" data-direccion="{{$cliente->clidir}}" data-clinom="{{$cliente->clinom}}" data-correo="{{$cliente->clicor}}" data-vendedor="{{$cliente->vendedor}}" data-telefono="{{$cliente->telefono}}">{{$cliente->clinum}} - {{$cliente->clinom}}</option>
       ...
       <!-- <div class="box-header" style="background-color:blue;">
          ...
          <div class="row">
             <div class="col-lg-6">
              <button type="button" id="btnRegComp" class=" btn btn-block btn-primary btn-lg botones">ACTUALIZAR</button><br>
            </div>

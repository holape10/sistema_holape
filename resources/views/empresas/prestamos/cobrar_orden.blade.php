...
if(pago=='0.00' || pago=='0' || pago==''){
 ...
 function presentaciones(id){
   ...
   function agregaritem_pre(button){
    ...
    $('#grdet').append('<tr>'+
    '<td width="900px"><input type="text" class="form-control" name="pronom[]" value="'+productonom+'"></td>'+
    '<td><input type="number" step="any" min="0" value="1" name="cant[]" onkeyup="Calcular(this);" onchange="Calcular(this);" class="form-control input-sm cant" id="font-size" style="width:60px"> </td>'+
    ...
    '<td><input  type="text" class="form-control" name="itemtotal[]"  value="'+precio+'" onkeyup="CalcularItem(this);" style="width:80px"></td>'+
    '<td><input  type="number" readonly="readonly" step="any" min="0" class="form-control input-sm" name="precio[]"  value="'+precio+'" style="width:80px" ></td>'+
 ...
 function agregarnota(button){
  ...
  $('#grdet').append("<tr>"+
    ...
    "<td><input  type='text' class='form-control input-sm' name='itemtotal[]'  value='0.00' onkeyup='CalcularItem(this);' style='width:80px' ></td>"+
     '<td><input  type="number" readonly="readonly" step="any" min="0" class="form-control input-sm" name="precio[]"  value="0" style="width:80px" ></td>'+
  ...
  if(contar>0){
      ...
      $('#grdet').append("<tr>"+
        ...
        "<td><input  type='number' step='any' min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='"+precio+"' style='width:80px' ></td>"+
        "<td ><input  type='number' step='any' min='0' class='form-control input-sm' name='desc[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='0.00' style='width:80px' ></td>"+
        "<td><input  type='text' class='form-control input-sm' name='itemtotal[]'  value='"+precio+"' onkeyup='CalcularItem(this);' style='width:80px' ></td>"+
        "<td><input  type='number' readonly='readonly' step='any' min='0' class='form-control input-sm' name='precio[]'  value='"+precio+"' style='width:80px' ></td>"+
        "<td hidden='hidden'><input type='text' class='form-control' name='proid[]'  value='"+proid+"' readonly='readonly' ></td>"+
        "<td><button type='button' onClick='deleteRow(this);' class='btn btn-danger btn-sm remove'><span class='glyphicon glyphicon-minus'></span></button></td></tr>");
...
<div class="container-fluid" id="general">
   ...
   {!!Form::open(array('url'=>'/restaurantpunto','autocomplete'=>'off','method'=>'POST','name'=>'formfact','id'=>'formfact','role'=>'form','files'=>'true'))!!}
    ...
    <input type="hidden" name="opcion" id="opcion" value="0">
       <input type="hidden" name="cotid" id="cotid" value="{{$cotizacion->IdCpe_cabecera}}">
                    ...
                    <div class="box-tools pull-right">
                      <a  data-target="#modal-guia" data-toggle="modal"><button type="button" class="btn btn-success btn-sm">GUIA</button></a>
                        <a  data-target="#modal-gasto" data-toggle="modal"><button type="button" class="btn btn-warning btn-sm">GASTO / INGRESO</button></a>
                         ...
                         <a  data-target="#modal-vehiculo" data-toggle="modal"><button type="button" class="btn btn-primary btn-sm">DATOS VEHICULO</button></a>
                ...
                <div class="box-header with-border form-group-sm">
                  ...
                  <div  class="col-lg-10">
                    <div class="form-group form-group-sm" id="divactpro">
                        <select data-tags='true' style=" font-weight: bold;" autocomplete="false" class="form-control" onkeypress="if(event.keyCode == 13) agregaritem();" onchange="agregaritem();"  name="producto" id="producto">
                 ...
                 <div class="box-body">
                   <table class="table table-hover" id="grdet">
                    ...
                    <tbody>
                    		...
                    		<tr><td width='900px'><input type='text' class='form-control' name='pronom[]' value='{{$det->cdedes}}'></td><td> <input type='number' step='any' min='0' value='{{$det->cdecan}}' name='cant[]' onkeyup='Calcular(this);' onchange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'> </td><td ><input readonly="readonly"  value="{{$det->umecod}}" style="width:100px" name="unid[]""  class="form-control input-sm"></td><td><input  type='number' step='any' min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='{{$det->cdepuni}}' style='width:80px' ></td><td> @if($datos->tipo_desc=='1') <input  type="number" step="any" min="0" class="form-control input-sm" name="desc[]" onChange="Calcular(this);"  onkeyup="Calcular(this);" value="{{$det->desc_mon}}" style="width:80px" > @else <input  type="number" step="any" min="0" class="form-control input-sm" name="desc[]" onChange="Calcular(this);"  onkeyup="Calcular(this);" value="{{$det->por_des}}" style="width:80px"> @endif</td><td><input  type='text' class='form-control' name='itemtotal[]'  value='{{$det->cdevve}}' onkeyup='CalcularItem(this);' style='width:80px' ></td><td><input  type="number" readonly="readonly" step="any" min="0" class="form-control input-sm" name="precio[]"  value="{{$det->cdepuni}}" style="width:80px" ></td><td hidden='hidden'><input type='text' class='form-control' name='proid[]'  value='@if(empty($det->IdProducto))  0 @else {{$det->IdProducto}} @endif' readonly='readonly' ></td><td><button type='button' onClick='deleteRow(this);' class='btn btn-danger btn-sm remove'><span class='glyphicon glyphicon-minus'></span></button></td><td hidden="hidden"><input type="hidden" readonly="readonly" name="IdCpe_detalle[]" value="{{$det->IdCpe_detalle}}"></td></tr>
                    	@endforeach
                    </tbody>
                  </table>
                ...
                <div class="form-group form-group-sm">
                    ...
                    <select name="estadopago" id="estadopago" class="form-control">
                      @foreach($creditos as $cre)
                       @if($cre->cre_dia_id == $cotizacion->cre_dia_id)
             ...
             <div class="row form-group form-group-sm">
              ...
              <div hidden="hidden" class="col-lg-2">
                <div class="form-group form-group-sm">
                  ...
                  <input type="text"  name="clinum" id="clinum" value="{{$cotizacion->ccandi}}"  placeholder="" class="form-control" >
             ...
             <div class="col-lg-3" >
              <div class="form-group">
                ...
                <select class="form-control selectpicker input-sm" data-show-subtext="true" data-live-search="true" name="clicod" id="clicod" onchange="seleccionarcliente();">
                  ...
                  @if($cliente->clicod == $cotizacion->clicod)
                ...
                <input type="hidden" readonly="readonly" name="clinom" id="clinom" value="{{$cotizacion->ccanom}}">
              </div>
            </div>
              ...
              <div class="col-lg-4">
                <div class="form-group form-group-sm">
                  ...
                  <input name="clidir" id="clidir" value="{{$cotizacion->direccion}}" class="form-control">
                </div>
              </div>
          ...
          <div class="row">
            <div class="col-lg-3">
              <div class="form-group-sm">
                <label>Total</label>
                <input type="text" class="form-control" style="font-size:16pt;font-weight:bold;" id="total" name="total" value='{{$cotizacion->ccaitv}}' readonly="readonly">
              </div>
            </div>
...
@include('empresas.puntosventas.modalmodificardatosvehiculo')
{!!Form::close()!!}
</div>

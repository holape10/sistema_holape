						...
						<table id="dtHorizontalExample" style="width:100%;font-size:9pt;"  class="table table-responsive table-striped table-bordered table-sm">
							...
							<thead>
								<tr>
									<th colspan="3" style="text-align:center;background-color:#337ab7;color:#fff;font-weight:bold;">RESUMEN DE VENTAS DESDE {{$fec_ini}} HASTA {{$fec_fin}} @if(!empty($dat_vend)) <br> {{$dat_vend->name}}  {{$dat_vend->apeusu}} @endif @if(!empty($dato_cliente)) <br> {{$dato_cliente->clinom}} @endif</th>
								...
								<tr>
									...
									<th style="width:100;text-align:center;">Vendedor</th>
							...
							<tbody>
								...
								@php
									$i=0;
								@endphp
								@foreach($ventas_res_vend as $cab)
									 	@php
											$i=$i+1;
										@endphp
									 	<tr>

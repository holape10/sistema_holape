						...
						<table id="dtHorizontalExample"  class="table table-responsive table-striped table-bordered table-sm" style="width:100%;font-size:8pt;">
							...
							<thead>
								<tr>
									<th colspan="10" style="text-align:center;background-color:#337ab7;color:#fff;"><strong>REPORTE DE GASTOS DETALLADOS DESDE {{Carbon::parse($fec_ini)->format('d-m-Y')}} HASTA {{Carbon::parse($fec_fin)->format('d-m-Y')}}</strong></th>
								...
								<tr>
									...
									<th style="width:70;text-align:center;">Detalle</th>								
								...
								@foreach($gastos_deta as $gast)
										...
										@endphp
									 	<tr>
											...
											<td style="width:30;text-align:right;">{{$gast->det_gasto}}</td>
											<td style="width:30;text-align:right;">{{number_format($gast->total,'2','.','')}}</td>

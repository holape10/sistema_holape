						...
						<table id="dtHorizontalExample"  class="table table-responsive table-striped table-bordered table-sm">
							<thead>
								<tr>
									<th colspan="8"><center><strong>REPORTE DE PEDIDOS DESDE: {{$fecin}} HASTA {{$fecfin}} @if(!empty($dat_ven)) - Vendedor: {{$dat_ven->name}} {{$dat_ven->apeusu}} @endif @if(!empty($dat_cli)) - Cliente: {{$dat_cli->clinom}} @endif   </strong></center>
									<th>TOTAL VENTAS </th>
								</tr>
								<tr>
									<th colspan="8"></th>

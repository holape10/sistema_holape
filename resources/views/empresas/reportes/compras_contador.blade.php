						...
						<table id="dtHorizontalExample"  class="table table-responsive table-striped table-bordered table-sm" style="font-size:8pt;">
							...
							<thead>
								<tr>
									<th colspan="21"><center><strong>COMPRAS DESDE {{$fecin}} HASTA {{$fecfin}} </strong></center></th>
							...
							<tbody>
								...
								<tr>
									...
									<td>{{$comprobante->prov_raz}}</td>
								 	<td>0.00</td>
									<td>@if($comprobante->gravado=='1'){{$comprobante->subtot_com}} @else 0.00 @endif</td>
									<td>@if($comprobante->gravado=='0'){{$comprobante->subtot_com}} @else 0.00 @endif</td>
									<td>0.00</td>

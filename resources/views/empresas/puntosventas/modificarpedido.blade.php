            ...
            success: function(response) { 
                ...
                if(response.mensaje){
                   alert(response.mensaje);
                     window.location.href = "/modificarpedido/"+mesa;
                }else{
    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
      ...
      @if(session()->has('success'))
        <div class="alert alert-success">
          <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
        <strong>Informaci√≥n!</strong> {{ session('success') }}
      </div>
      @endif
  </div>
</div>
 <div class="row">
  ...
  <div class="box">
   <div class="box-header with-border">
    ...
    <div class="box-body">
    {!!Form::open(array('url'=>'/mesaadicionar','autocomplete'=>'off','method'=>'POST','name'=>'formfact','id'=>'formfact','role'=>'form','files'=>'true'))!!}
    {{Form::token()}}
      ...
      <td style='width:500px;'>
        <input type='text' class='form-control input-sm' name='pronom[]' value='{{$p->pronom}}' readonly='readonly'>
      </td>
      <td>
        <input type='number' step="any" min='0' value='{{$p->cantidad}}' name='cant[]' onkeyup="Calcular(this);" onChange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'>
      </td>
      <td hidden="hidden">
        <select style='width:100px' name='unid[]'  class='form-control input-sm'>
        @foreach($unidades as $und)
        @if($und->umecod == $p->unidad)
        ...
        @endif @endforeach
         ...
         <td hidden='hidden'>
             <input type='text' class='form-control input-sm' name='provun[]'  value='{{$p->provunitem}}' readonly='readonly' style='width:130px' >
       </td>
      <td >
        <input type='number' step="any" min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  value='{{$p->propunitem}}' onkeyup='Calcular(this);' style='width:70px' >
      </td>
      <td hidden="hidden">
        <input type='text' class='form-control input-sm' name='itemtotal[]'  value='{{$p->totalitem}}' readonly='readonly' style='width:130px' >
      </td>
      <td hidden='hidden'>
        <input type='text' class='form-control input-sm' name='proid[]'  value='{{$p->IdProducto}}' readonly='readonly' style='width:130px' >
      </td>
      <td style='width:200px;'>
        <input type='text' class='form-control input-sm ' name='detalle[]' value='{{$p->detalle}}'>
      </td>
        <td hidden="hidden" style='width:200px;'>
        <input type='text' class='form-control input-sm' name='iddetalle[]' value='{{$p->ped_det_id}}'>
      </td>
      <td>
        <a href="" data-target="#modal-deleteitem-{{$p->ped_det_id}}" data-toggle="modal"><button type="button" class=" btn btn-block btn-danger btn-sm botones" ><span class='glyphicon glyphicon-minus'></span></button></a>
  ...
  @endforeach
     ...
     @else
       <div class="alert alert-danger">
          <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
        <strong>EL PEDIDO SE EST¡ FACTURANDO EN CAJA</strong>
      </div>
     @endif
...
<div class="col-lg-6">
  <div class="box">
   <div class="box-header with-border form-group-sm">
      <input class="form-control" name="buscarproducto" id="buscarproducto" placeholder="CÛdigo Barras o descripcion">

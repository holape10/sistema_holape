  ...
  <script>
   ...
   function Calculargasto(){
      ...
      $("#detgasto tbody tr").each(function(){
          ...
          item = parseFloat($(this).find("td:eq(1)  > input").val());
    ...
    $("#btnRegComp").on("click", function() {
      ...
      $.ajax({
        ...
        if(respuesta.estado =='error'){
           ...
           alert("PROFORMA NUMERO: "+respuesta.pedido);
...
function Calcular(ele) {
  ...
  $(tr).each(function() {
    ...
    totitemgrav = $(this).find("td:eq(1) > input").val() * ($(this).find("td:eq(3) > input").val()-$(this).find("td:eq(4) > input").val());
...
if(pago=='0.00' || pago=='0' || pago==''){
 ...
 function presentaciones(id){
  ...
  if(contar>0){
      ...
      $('#grdet').append("<tr><td width='900px'><input type='text' class='form-control' name='pronom[]' value='"+producto+"'></td><td> <input type='number' step='any' min='0' value='1' name='cant[]' onkeyup='Calcular(this);' onchange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'> </td><td hidden='hidden'><select style='width:100px' name='unid[]'  class='form-control input-sm'> @foreach($unidades as $und) @if($und->umecod == 'UNI') <option  selected='selected' value='{{$und->umecod}}'>{{$und->umenom}}</option> @else <option  value='{{$und->umecod}}'>{{$und->umenom}}</option> @endif @endforeach </select></td><td><input  type='number' step='any' min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='"+precio+"' style='width:80px' ></td><td><input  type='number' step='any' min='0' class='form-control input-sm' name='desc[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='0.00' style='width:80px' ></td><td><input readonly='readonly' type='text' class='form-control' name='itemtotal[]'  value='"+precio+"' onkeyup='CalcularItem(this);' style='width:80px' ></td><td hidden='hidden'><input type='text' class='form-control' name='proid[]'  value='"+proid+"' readonly='readonly' ></td><td><button type='button' onClick='deleteRow(this);' class='btn btn-danger btn-sm remove'><span class='glyphicon glyphicon-minus'></span></button></td><td hidden='hidden'><input  type='number' step='any'  class='form-control input-sm' name='propunref[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' min='"+precio+"' value='"+precio+"' style='width:80px' ></td></tr>");
  ...
  function buscarpedido(){
    ...
    $("#divcargar").html('<center><img src="/img/load.gif" width="100px" height="100px"></center>');
    $.ajax({
      type: "GET",
      dataType: 'json',
...
<div class="col-lg-6">
  <div class="col-lg-12">
        <div class="box">
         ...
         <div class="box-header" style="background-color:blue;">
              <font color="white" size="3"><center><strong>DATOS DE LA PROFORMA N° {{$cabecera->numdoc}}</strong></center></font>
         </div>
         <div class="box-body">
             ...
             <div  class="row">
              <div class="col-lg-5">
                <div class="form-group form-group-sm">
                  ...
                  <label >
                    <input type="radio" name="tdocod" id="pedido" value="15" checked="checked" > PRO
                  </label>

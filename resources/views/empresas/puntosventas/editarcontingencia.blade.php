...
<body>
  ...
  function agregarcuota(){
   ...
   $("#btnRegCompAct").on("click", function() {
    ...
    $("#btnRegCompRegAct").on("click", function() {
    ...
    $("#buscarproducto").keypress(function(e) {
      ...
      var producto = $("#buscarproducto").val();
      if (e.which == 13) {

      $.ajax({
        type: "GET",
        dataType: 'json',
        url: '/consultarproductosbarra/'+producto,
      }).done(function(respuesta){
   ...
   /* var producto = $('#producto').select2('data')[0].producto;
    ...
    var precio =  respuesta[0].propun;
    var proid =  respuesta[0].id;
    var unidad =  respuesta[0].unidad;
    var cantidad = '1';
    var id_almacen_pro = respuesta[0].id_almacen;
...
function Calcular(ele) {
  ...
  $(tr).each(function() {
    ...
    if(val_desc>0){
      ...
      if(tipo_desc =='2'){
        descuento = ($(this).find("td:eq(4) > input").val()*(val_desc/100));
      }
...
if(pago=='0.00' || pago=='0' || pago==''){
 ...
 function agregarnota(button){
  ...
  $('#grdet').append("<tr>"+
     "<td width='250px'><input type='text' class='form-control input-sm' readonly='readonly'  value=''></td>"+
    "<td width='900px'><input type='text' class='form-control input-sm btn-block' name='pronom[]' value='' ></td>"+
    ...
    "<td width='80PX'><select   name='unid[]'  class='form-control input-sm'>@foreach($unidades as $uni) @if($uni->umecod =='NIU') <option selected='selected' value='{{$uni->umecod}}'>{{$uni->umenom}}</option> @else <option  value='{{$uni->umecod}}'>{{$uni->umenom}}</option> @endif @endforeach</select></td>"+
  ...
  if(precio_ref===undefined){
      $('#grdet').append("<tr>"+
        "<td width='250px'><input type='text' readonly='readonly' class='form-control input-sm'  value='"+codigo+"'></td>"+
        "<td width='900px'><input type='text' class='form-control input-sm' name='pronom[]' value='"+producto+"'></td>"+
        "<td> <input type='number' step='any' min='0' value='"+cantidad+"' name='cant[]' onkeyup='Calcular(this);' onchange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'></td>"+
        "<td width='80px'><input readonly='readonly'  value='"+unidad+"' name='unid[]'  class='form-control input-sm'></td>"+
        "<td><input  type='number' step='any' min='0' class='form-control input-sm' readonly='readonly' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='"+precio+"'  style='width:80px' ></td>"+
                   ...
                   <table class="table table-hover" id="grdet">
                    ...
                    <tbody>
                      @foreach($detalle as $det)
                        <tr><td width='250px'><input type='text' readonly='readonly' class='form-control input-sm'  value='{{$det->procod}}'></td><td width='900px'><input type='text' class='form-control input-sm' name='pronom[]' value='{{$det->cdedes}}'></td><td> <input type='number' step='any' min='0' value='{{$det->cdecan}}' name='cant[]' onkeyup='Calcular(this);' onchange='Calcular(this);' class='form-control input-sm ' id='font-size' style='width:60px'> </td><td width="80px"><input readonly="readonly"  value="{{$det->umecod}}" name="unid[]"  class="form-control input-sm"></td><td><input  type='number' step='any' min='0' class='form-control input-sm' name='propun[]' onChange='Calcular(this);'  onkeyup='Calcular(this);' value='{{$det->cdepuni}}' readonly="readonly" style='width:80px' ></td> <td> @if($datos->tipo_desc=='1') <input  type="number" step="any" min="0" class="form-control input-sm" name="desc[]" onChange="Calcular(this);"  onkeyup="Calcular(this);" value="{{$det->desc_mon}}" style="width:80px" > @else <input  type="number" step="any" min="0" class="form-control input-sm" name="desc[]" onChange="Calcular(this);"  onkeyup="Calcular(this);" value="{{$det->por_des}}" style="width:80px"> @endif</td><td><input  type='text' class='form-control input-sm' name='itemtotal[]' readonly="readonly" value='{{$det->cdevve}}' onkeyup='CalcularItem(this);' style='width:80px' ></td><td hidden="hidden"><input  type="number" readonly="readonly" step="any" min="0" class="form-control input-sm" name="precio[]"  value="{{$det->cdepuni}}" style="width:80px" ></td><td hidden='hidden'><input type='text' class='form-control' name='proid[]'  value='@if(empty($det->IdProducto))  0 @else {{$det->IdProducto}} @endif' readonly='readonly' ></td><td hidden='hidden' ><input type='text' class='form-control' name='id_almacen_pro[]'  value='{{$det->id_almacen_pro}}' readonly='readonly' ></td><td><button type='button' onClick='deleteRow(this);' class='btn btn-danger btn-sm remove'><span class='glyphicon glyphicon-minus'></span></button></td><td hidden="hidden"><input type="hidden" readonly="readonly" name="IdCpe_detalle[]" value="{{$det->IdCpe_detalle}}"></td></tr>
                      @endforeach
                    </tbody>
                  </table>
...
<div class="col-lg-5">
  <div class="col-lg-12">
        <div class="box">
         ...
         <div class="box-header" style="background-color:blue;">
              <font color="white"><center><strong>COMPROBANTE DE CONTINGENCIA</strong></center></font>
         </div>
         <div class="box-body">
        ...
        <input type="hidden" readonly="readonly" name="ped_id" value="{{$cabecera->IdCpe_cabecera}}">
                ...
                <div class="form-group form-group-sm">
                    ...
                    <select name="id_almacen" id="id_almacen" class="form-control">
                      @foreach($almacenes as $alm)
                       @if($alm->id_almacen == $almacen->id_almacen)
                ...
                <div class="form-group form-group-sm">
                  ...
                  @endif
                   ...
                   @if($cabecera->tdocod=='01')
                     <label  >
                    <input type="radio" name="tdocod" id="factura" value="01" checked="checked"> FA
                  </label>
                  @else
                    <label  >
                  ...
                  @endif
                   ...
                   @if($cabecera->tdocod=='13')
                     <label>
                     <input type="radio" name="tdocod" id="nota" value="13" checked="checked"  > NV
                  </label>
                  @else
                     <label>
            ...
            <div class="col-lg-4">
               <div class="form-group form-group-sm">
                <label>SERIE</label>
                <input type="text" name="txt_serdoc" value="{{$cabecera->serdoc}}" class="form-control">
             </div>
            </div>
            <div class="col-lg-4">
               <div class="form-group form-group-sm">
                <label>NUMERO</label>
                <input type="text" name="txt_numdoc" value="{{$cabecera->numdoc}}" class="form-control">
             </div>
            </div>
           ...
           <div class="row">
            <div class="col-lg-4">
               <div class="form-group form-group-sm">
                <label>GUI REMISI&Oacute;N</label>
                <input type="text" name="guia_remision" value="{{$cabecera->guia_remision}}" class="form-control">
             </div>
            </div>
            ...
            <div class="col-lg-4">
               <div class="form-group form-group-sm">
                <label>PLACA</label>
                <input type="text" name="placa_comp" value="{{$cabecera->placa}}" class="form-control">
             </div>
            </div>
         ...
         <div class="box-body" id="divcliente">
             ...
             <div class="col-lg-3" >
              <div class="form-group">
                ...
                <input hidden="hidden"  readonly="readonly" name="clinom" id="clinom" value="{{$cabecera->ccanom}}">
              </div>
            </div>
        ...
        <div class="box-header" style="background-color:blue;">
          ...
          <div class="row">
              @if( $cabecera->tdocod =='16')
            <div class="col-lg-4">
           
            
               <button type="button" id="btnRegComp" class=" btn btn-block btn-success btn-lg botones">COBRAR</button>
       ...
       <br>
         ...
         @endif
             ...
             @if($cabecera->tdocod =='01' || $cabecera->tdocod =='03'  || $cabecera->tdocod =='13' || $cabecera->tdocod =='07' || $cabecera->tdocod =='15')
             ...
             @else
               <button type="button" id="btnRegCompReg" class=" btn btn-block btn-primary btn-lg botones">REGISTRAR</button>
             @endif
             <br>
            </div>

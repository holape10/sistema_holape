          ...
          $('.codpro').on('change',function () {
               ...
               $(".codpro").autocomplete({
                  ...
                  select: function(event,ui) {
                     ...
                     $(this).closest('tr').find("td:eq(7) > input").val(ui.item.provun);
                     $(this).closest('tr').find("td:eq(8) > input").val(ui.item.propun);
					 $(this).closest('tr').find("td:eq(11) > input").val(ui.item.propun);
                     ...
                     $(this).closest('tr').find("td:eq(7) > input").prop("readonly",true);
				 ...
				 $(".detpro").autocomplete({
                   source: '{!!URL::route('consultarproductonomsinstock')!!}',
                  dataType: "json",
                  minLength: 1,
                  autoFocus:true,
                  select: function(event,ui) {   
					 ...
					 $(this).closest('tr').find("td:eq(11) > input").val('0');
                     $(this).closest('tr').find("td:eq(1) > select").val(ui.item.umecod).attr('selected', 'selected');
                     $(this).closest('tr').find("td:eq(2) > input").prop("readonly",true);
                     $(this).closest('tr').find("td:eq(3) > input").prop("readonly",false);
                     $(this).closest('tr').find("td:eq(6) > input").prop("readonly",true);
...
function Calcular(ele) {
  ...
  $(tr).each(function() {
      ...
      var tigv = $(this).find("td:eq(6) > select").val();
      var precigv = $(this).find("td:eq(8) > input").val();
      var cantidad =$(this).find("td:eq(0) > input").val();

       if(tigv=='1'){

        //totitemgrav = $(this).find("td:eq(0) > input").val() * $(this).find("td:eq(7) > input").val();
        ...
        $(this).find("td:eq(7) > input").val(presigv.toFixed(10));
        $(this).find("td:eq(10) > input").val(subtotal.toFixed(2));
        $(this).find("td:eq(9) > input").val(igvitem.toFixed(2));
       ...
       if (tigv=='8')
       {
        ...
        totitemexon = $(this).find("td:eq(0) > input").val() * $(this).find("td:eq(7) > input").val();
        ...
        $(this).find("td:eq(7) > input").val(presigv.toFixed(2));
       ...
       {
            totitemgrat = $(this).find("td:eq(0) > input").val() * $(this).find("td:eq(7) > input").val();
            presigv = (precigv/1);
            subtotal = presigv*cantidad;
            total = subtotal*1;
            igvitem = 0;
       ...
       if (tigv=='9' || tigv=='16')
       {
          totiteminaf = $(this).find("td:eq(0) > input").val() * $(this).find("td:eq(7) > input").val();
          presigv = (precigv/1);
            subtotal = presigv*cantidad;
            total = subtotal*1;
            igvitem = 0;
        ...
        $(this).find("td:eq(7) > input").val(presigv);
...
function calculartotal(){
   ...
   var $svalor=0;
    $("#detFact tbody tr").each(function(){
    //sum= sum + parseFloat($(this).find("td:eq(7) > input").val()) ;
       ...
       if(tigv=='1'){
        ...
        totgrav = totgrav + parseFloat($(this).find("td:eq(10)  > input").val());
       }

       if (tigv=='8')
       {
            totexon = totexon + parseFloat($(this).find("td:eq(10) > input").val());
       }

       if (tigv=='10' || tigv=='11' || tigv=='12' || tigv=='13' || tigv=='14' || tigv=='15' || tigv=='2' || tigv=='3' || tigv=='4' || tigv=='5' || tigv=='6' || tigv=='7')
       {
            totgrat = totgrat + parseFloat($(this).find("td:eq(10) > input").val());

       }

       if (tigv=='9' || tigv=='16')
       {
            totinaf = totinaf + parseFloat($(this).find("td:eq(10) > input").val());

       }


        totigv = totigv + parseFloat($(this).find("td:eq(9) > input").val());
        ...
        <div class="col-lg-12">
            <div class="btn-toolbar" role="toolbar" aria-label="...">
                <div class="btn-group" >
                        ...
                        <a href="/movgastos"><button type="button"  class=" btn btn-success btn-sm"><span class="glyphicon glyphicon-search"></span> Consultar Comprobantes</button></a>
                </div>
               	<div class="btn-group" >
        ...
        <div class="container-fluid detalle">
            <div class="row">
                ...
                <div class="panel panel-default">
                  ...
                  <td>
                                    <input size='40' type="text" id="lote" name="lote[]"  class="form-control input-sm">
                                   </td>
                                  <td>
                                    <input  type="date" id="fecvenc" name="fecvenc[]" value="{{Carbon::now()->format('Y-m-d')}}" class="form-control input-sm">
                                   </td>
                ...
                <div class="col-lg-9">
                    ...
                    <button type="submit" id="btn" name="btn"  class="btn btn-primary"><strong>REGISTRAR GASTO</strong></button>

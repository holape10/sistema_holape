...
<style>
    ...
    #formfact label.error { color:red; }
...
$(document).ready(function() {
    ...
    // Validaciones e inicializaciones adicionales
    ...
    $('.detpro, .cant').on('change',function () {
        validartabla();
    });
    ...
    $("#transportistanum").autocomplete({
        ...
        select: function(event,ui) {
            ...
            $("#transportistatdicod").val(ui.item.tdicod).attr('selected', 'selected');
        }
    });
    ...
    $("#conductornum").autocomplete({
        ...
        select: function(event,ui) {
            ...
            $("#conductortdicod").val(ui.item.tdicod).attr('selected', 'selected');
        }
    });
    ...
    $('#serdoc, #numdoc').on('change', function() {
        $('#codunique').val($('#txt_IdEmpresa').val()+''+$('#txt_tdocod').val()+''+$('#serdoc').val() + '' +parseInt($('#numdoc').val(),10));
    });
    ...
    $('#formfact').validate({
        rules: {
            tdicod: { required:true },
            serdoc: { required:true, alphanumeric:true, maxlength: 4 },
            fecEmi: { required: true, date: true },
            fecEnv: { date: true },
            numdoc: { required:true, digits:true, maxlength:8 },
            Descuentos: { number:true, min:0 },
            otros: { number:true, min:0 },
            otrosc: { number:true, min:0 },
            camdoc: {
                required : function () { return $('#formfact select[name="mondoc"]').val() != 'PEN'; },
                min: {
                    param: 1,
                    depends: function () { return $('#formfact select[name="mondoc"]').val() != 'PEN'; }
                }
            },
            fecVen: "required",
            clinum: { required:true, digits:true, maxlength:11 },
            clinom: "required",
            clidir: "required",
            clicor: { email:true },
            obser: { maxlength: 250 }
        ...
        messages: {
            tdicod: { required:"Elegir el tipo de documento" },
            serdoc: { required:"Ingresar la serie", alphanumeric:"Serie válida", maxlength:"Máximo 4 digitos" },
            camdoc: { required:"Ingresar tipo de cambio", min:"Debe ser mayor a 0" },
            numdoc: { required:"Ingresar número", digits:"N° válido", maxlength:"Máximo 8 dígitos" },
            fecEmi: { required:"Ingresar fecha de emisión", date:"Fecha válida" },
            clinum: { required:"Ingresar N° Documento", digits:"N° válido", maxlength:"Máximo 11 dígitos" },
            clinom: "Ingresar el nombre",
            clidir: "Ingresar la dirección"
        }
    });
    ...
    $('#add').click(function() { agregarlinea(); });
    $('#addm').click(function() { agregarlineamayor(); });
    $("#cambia").click(function(){ $("#texto").toggle(1000); });
    $("#addgr").click(function(){ $('#gremi').show(); });
    ...
    $( "#formfact" ).submit(function( event ) {
        ...
        $('#detFact >tbody >tr').each(function(){
            ...
            if(pro==''){ conpro++; } else if(det==''){ condet++; } else if(cant<1){ concant++; }
        });

        if(conpro>0){ $('.alertpro').show(); event.preventDefault(); } else { $('#alertpro').hide(); }
        if(condet>0) { $('.alertdet').show(); event.preventDefault(); } else { $('#alertdet').hide(); }
        if(concant>0){ $('.alertcant').show(); event.preventDefault(); } else { $('#alertcant').hide(); }
    ...
    $('#fecEmi').on('change', function() { $('#camdoc').val(0); });

    $('#clinum').on('dblclick', function() {
        $('#clinum, #clinom, #clidir, #clicor').prop("readonly",false);
        $('#clinum, #clinom, #clicor').val("");
        $('#clidir').val("--");
    });
});
</script>
...
function deleteRow(btn) {
    ...
    calculartotal();
    calcularvuelto();
    if ($('#detFact >tbody >tr').length == 0){ $('.alertitem').show(); }
    validartabla();
}
...
function Calcular(ele) {
    ...
    $(tr).each(function() {
        ...
        // Lógica de IGV omitida por brevedad según original
    });
    calculartotal();
    calcularvuelto();
}
...
function agregaritem(){
    ...
    var procod = $('#cod_producto').val();
    var unidad = $('#uni_producto').val();
    var cantidad = $('#can_producto').val();
    var id_almacen_pro = $('#id_almacen_pro').val();

    $('#detFact').append('<tr>'+
        '<td><input type="text" name="codpro[]" id="codpro" style="background-color:#ABEBC6" value="'+procod+'" class="codpro form-control input-sm"></td>'+
        '<td><input onkeypress="if (event.keyCode == 13) enviar_formulario(); if(event.keyCode == 45) deleteRow(this);" class="detpro form-control input-sm" name="detpro[]" id="detpro" size="100" onChange="Calcular(this);" value="'+producto+'" OnKeyUp="Calcular(this)"; onfocus="Calcular(this)"; ></td>'+
        '<td><input type="number" step=".00001" id="cant" size="10" name="cant[]" value="'+cantidad+'" OnKeyUp="Calcular(this);" onKeypress="if(event.keyCode == 45) deleteRow(this);" class="cant form-control input-sm" /></td>'+
        '<td><input class="form-control input-sm" value="'+unidad+'" readonly="readonly" name="unid[]"></td>'+
        '<td hidden="hidden"><input type="text" name="IdProducto[]" id="IdProducto" style="background-color:#ABEBC6" value="'+proid+'" class="codpro form-control input-sm"></td>'+
        '<td><button type="button" onClick="deleteRow(this);" class="btn btn-danger btn-sm remove"><span class="glyphicon glyphicon-minus"></span></button></td></tr>');
...
function ingresar_cantidad_precio(){
    ...
    if(contar>0){
        ...
        $('#modal-cantidad-precio').on('shown', function(){ $("#can_producto").focus(); });
...
function validartabla(){
    ...
    if(conpro>0){ $('.alertpro').show(); } else { $('#alertpro').hide(); }
    if(condet>0){ $('.alertdet').show(); } else { $('#alertdet').hide(); }
    if(concant>0){ $('.alertcant').show(); } else { $('#alertcant').hide(); }
}
...
function buscarcliente() {
    var documento = $("#clinum").val();
    
    if (documento.length !== 8 && documento.length !== 11) {
        alert("El documento debe tener 8 o 11 dígitos válidos.");
        return;
    }
    ...
    $.ajax({
        ...
        $('#clinom').val(respuesta.nom);

        // Si es DNI (8 dígitos), el tipo de documento es 1
        if (documento.length === 8) {
            $('#tdicod').val('1');
        }

        // Si es RUC (11 dígitos), el tipo de documento es 6 (SUNAT)
        if (documento.length === 11) {
            $('#tdicod').val('6'); // <-- CORRECCIÓN APLICADA AQUÍ
            $('#clidir').val(respuesta.dir);
            $('#direccionllegada').val(respuesta.dir);

            if (respuesta.ubigeo) {
                $('#ubigeollegada').val(respuesta.ubigeo);
                $('#ubigeollegada').selectpicker('refresh');
            }
        }
    }).fail(function() {
        $("#imgloadcliente").hide();
        alert("Ocurrió un error de conexión al consultar el documento.");
    });
}

function buscarconductor() {
    var documento = $("#conductornum").val();
    if (documento.length !== 8) {
        alert("El DNI del conductor debe tener 8 dígitos.");
        return;
    }
    $("#imgloadcliente").show(); 
    $.ajax({
        type: "POST",
        url: '/consultarDocumento',
        data: { documento: documento, _token: $('input[name="_token"]').val() },
        dataType: 'json'
    }).done(function(respuesta) {
        $("#imgloadcliente").hide();
        if (respuesta.error) { alert(respuesta.error); return; }
        $('#conductornom').val(respuesta.nom);
    }).fail(function() {
        $("#imgloadcliente").hide();
        alert("Ocurrió un error de conexión al consultar el conductor.");
    });
}
...
<br><div class="container">
    ...
    <div hidden="hidden" class="container-fluid">
        <div class="col-lg-12">
            <div class="btn-toolbar" role="toolbar" aria-label="...">
                ...
                <div class="btn-group">
                    <a href="/guiasremision/create"><button type="button" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-plus"></span> Nueva Guía</button></a>
                </div>
            </div>
        </div>
    </div>
    ...
    {{Form::token()}}
        ...
        @if(!empty($cabecera))
         <input type="hidden" readonly="readonly" name="id_doc_ref" value="{{$cabecera->IdCpe_cabecera}}">
        @else
         <input type="hidden" readonly="readonly" name="id_doc_ref" value="0">
        @endif
        ...
        <div class="container-fluid">
                ...
                <div class="col-lg-12">
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <div class="col-lg-2" id="div_almacen" style="display:none;">
                            ...
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group form-group-sm">
                                    ...
                                    <input name="serie_num_ref" value="" id="serie_num_ref" class="form-control" placeholder="SERIE-NUMERO">
                                </div>
                            </div>
                        ...
                        <div class="row">
                                ...
                                <div class="form-group form-group-sm">
                                    ...
                                    <select name="tdicod" id="tdicod" class="form-control">
                                        @foreach($docidentidad as $doc)
                                            @if(!empty($cabecera))
                                                <option value='{{$doc->tdicod}}' {{ ($doc->tdicod == $cabecera->tdicod || old('tdicod') == $doc->tdicod) ? 'selected' : '' }}>{{$doc->tdides}}</option>
                                            @else
                                                <option value='{{$doc->tdicod}}' {{ ($doc->tdicod == '1' || old('tdicod') == $doc->tdicod) ? 'selected' : '' }}>{{$doc->tdides}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="form-group form-group-sm">
                                    ...
                                    <input type="text" name="clinum" id="clinum" value="{{ !empty($cabecera) ? $cabecera->ccandi : '00000000' }}" onKeypress="if(event.keyCode == 13) buscarcliente();" class="form-control">
                                </div>
                            </div>
                            ...
                            <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                <div class="form-group form-group-sm">
                                    ...
                                    <input type="text" name="clinom" id="clinom" value="{{ !empty($cabecera) ? $cabecera->ccanom : 'Varios' }}" class="form-control">
                                </div>
                            </div>
            ...
            <div class="box">
                <div class="box-header" style="background:#337ab7;">
                    <font color="white"><strong>Dirección de Partida</strong></font>
                </div>
                <div class="box-body">
                    <div class="row">
                            ...
                            <div class="form-group form-group-sm">
                                ...
                                <select class="form-control selectpicker input-sm" data-show-subtext="true" data-live-search="true" name="ubigeopartida" id="ubigeopartida">
                                    @foreach($ubigeos as $ubi)
                                        <option value="{{$ubi->ubi_cod}}" {{ (isset($senudoc->ubigeo) && $senudoc->ubigeo == $ubi->ubi_cod) ? 'selected' : '' }}>
                                            {{$ubi->ubi_des}}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-lg-7 col-md-7 col-sm-12 col-xs-12">
                            <div class="form-group form-group-sm">
                                <label>Dirección</label>
                                <input type="text" name="direccionpartida" id="direccionpartida" class="form-control" value="{{ $senudoc->direccion ?? '' }}">
                            </div>
                        </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12" id='div_cod_local_part'>
                            <div class="form-group form-group-sm">
                                ...
                                <input type="text" name="cod_local_part" id="cod_local_part" class="form-control" value="{{ $senudoc->codigofiscal ?? '' }}">
                            </div>
                        </div>
            ...
            <div class="col-lg-6">
                <div class="box">
                    <div class="box-header" style="background:#337ab7;"> 
                        <font color="white"><strong>Dirección de Llegada</strong></font>
                    </div>
                    <div class="box-body">
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12" id='div_cod_local_des'>
                                <div class="form-group form-group-sm">
                                    <label>Código Fiscal</label>
                                    <input type="text" name="cod_local_des" id="cod_local_des" class="form-control">
                                </div>
                            </div>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-6" id="div_dat_transp">
                    <div class="box">
                        <div class="box-header" style="background:#337ab7;"> 
                            <font color="white"><strong>Datos de Transportista</strong></font>
                        </div>
                        <div class="box-body">
                            <div class="row">
                                <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                ...
                <div class="col-lg-12">
                    <div class="box">
                        ...
                        <div class="box-body">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <table id="detFact" class="table table-hover">
                                        <thead>
                                            <th>Código</th>
                                          ...
                                          @if(!empty($items))
                                           @foreach($items as $item)
                                              <tr>
                                                ...
                                                <td><input type="number" step=".00001" id="cant" size="10" value="{{$item->cdecan}}" name="cant[]" OnKeyUp="Calcular(this);" onKeypress="if(event.keyCode == 45) deleteRow(this);" class="cant form-control input-sm" /></td>
                                                <td>
                                                  <select style="width:100px" name="unid[]" class="form-control input-sm"> 
                                                    @foreach($unidades as $und) 
                                                      <option value="{{$und->umecod}}" {{ $und->umecod == $item->umecod ? 'selected' : '' }}>{{$und->umenom}}</option> 
                                                    @endforeach 
                                                  </select>
                                                </td>
                            ...
                            <div class="row" style="display:none;" id="imgloadguia">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <div class="form-group form-group-sm">
                                        <center>
                                            <img width="300px" height="300px" src="/img/load.gif"><br>
                                            REGISTRANDO GUÍA...
                                        </center>
                                    </div>
                                </div>
                            ...
                            <div class="box-body" id="divcargar">
                                <div class="col-lg-8">
                                    <button type="button" name="btnregistrar" id="btnregistrar" class="btn btn-primary"><strong>REGISTRAR GUÍA</strong></button>
...
$(document).ready(function() {
    ...
    $("#modal-cantidad-precio").on('hidden.bs.modal', function () { actualizarpro(); });
    $("#producto").focus();
                ...
                return {
                    results: $.map(response, function(item) {
                        return {
                            "text": item.text,
                            "id": item.id,
                            "pro_rel": item.pro_rel,
                            "presentacion": item.contar,
                            "propun": item.propun,
                            "unidad": item.unidad,
                            "producto": item.producto,
                            "id_almacen_pro": item.id_almacen,
                            "pro_cod": item.codigo,
                            "icbper": item.icbper,
                            "mon_icbper": item.mon_icbper
                        }
                    })
                };
            },

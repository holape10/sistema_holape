   ...
   $(document).ready(function()
   {
    ...
    $("#btnRegComp").on("click", function() {
      ...
      $.ajax({
        type: "POST",
        dataType: 'json',
        url: '/actualizarotcompu',
        data: formulario,
      }).done(function(respuesta){


        window.location.href = "/ordenescompu";
  ...
  <div class="row"> 
    <div class="col-lg-12">
       ...
       <div class="box-header with-border" style="background:blue">
        <font size="2" color="white"><strong><center>EDITAR ORDEN TRABAJO</center></strong></font>
      </div>
      ...
      <div class="form-group form-group-sm">
        <label for="equipo">Equipos</label>
          <div class="input-group input-group-sm">
            ...
            <input type="text" readonly="readonly" name="txt_equipo" id="equi_des" value="{{$cotizacion->pronom}} {{$cotizacion->mar_nom}} {{$cotizacion->mod_nom}}"  class="form-control">
            <span class="input-group-btn">
              <button type="button" class="btn btn-primary btn-flat" onclick="seleccionarequipos();">BUSCAR EQUIPO</button>
            </span>
          </div>
        <input type="hidden" readonly="readonly" class="form-control" name="equi_id" id="equi_id" value="{{$cotizacion->equi_id}}">
      </div>
    </div>
        ...
        <div class="col-lg-2">
          <div class="form-group form-group-sm">
            ...
            <input  type="text" id="equi_ser" name="equi_ser" value="{{$cotizacion->equi_ser}}" class="form-control">
          </div>
        </div>
       ...
       <div class="col-lg-2">
            ...
            <div class="form-group form-group-sm">
              ...
              <input  type="date" id="fec_doc_gar" name="fec_doc_gar" value="{{$cotizacion->fec_doc_gar}}" class="form-control">
            </div>
          </div>
           <div class="col-lg-3">
         ...
         <select class="form-control selectpicker" data-show-subtext="true" data-live-search="true" name="tdocod_gar" >
           @foreach($comprobantes as $comprobante)
           	@if($cotizacion->tdocod_gar==$comprobante->tdocod)
           		<option selected="selected" value="{{$comprobante->tdocod}}">{{$comprobante->tdodes}}</option>
           	@else
        ...
        <div class="col-lg-2">
          <div class="form-group form-group-sm">
            ...
            <input  type="text" id="doc_gar" name="doc_gar" value="{{$cotizacion->doc_gar}}" class="form-control">
          </div>
        </div>
    ...
    <div class="box-body" id="divcliente">
      ...
      <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
        <div class="form-group form-group-sm">
          ...
          <input type="text"  name="clinum" id="clinum" value="{{$cotizacion->clinum}}" onKeypress="if(event.keyCode == 13) buscarclientetaller();" placeholder="" class="form-control" >
        ...
        <div class="row">
          ...
          <div class="row">
            ...
            <div class="form-group form-group-sm">
             ...
             @if(Auth::user()->hasRole('tecnico') || Auth::user()->hasRole('coordinador') ) 
             <textarea type="text"readonly="readonly"   class="form-control" rows="4">{{$cotizacion->observaciones}}</textarea> 
             @else
 					<textarea type="text" class="form-control" name="observaciones" rows="4">{{$cotizacion->observaciones}}</textarea> 
             @endif
            </div>
          </div>
    ...
    <div class="col-lg-12">
      <div class="form-group form-group-sm">
        <label>Estado F&iacute;sico </label>
          @if(Auth::user()->hasRole('tecnico') || Auth::user()->hasRole('coordinador') ) 
            <textarea type="text" name="est_fis" readonly="readonly" class="form-control" rows="4">{{$cotizacion->est_fis}}</textarea> 
             @else
 				<textarea type="text" name="est_fis" class="form-control" rows="4">{{$cotizacion->est_fis}}</textarea> 
             @endif
    ...
    <div class="col-lg-12">
      <div class="form-group form-group-sm">
        ...
        <textarea type="text" name="obs_act"  class="form-control" rows="4">{{$cotizacion->obs_act}}</textarea> 
      </div>
    </div>
    ...
    <div class="row">
      ...
      <table class="table table-hover" id="grdet">
        ...
        <tbody>
            @foreach($lis_tip_equi as $lte)
              <tr>
                <td hidden='hidden'><input type='text' name='id[]'  readonly='readonly'  class='form-control input-sm'  value=''></td>
                <td><select   name='id_tip_equi[]'  class='form-control input-sm'>@foreach($tipo_equipo as $te) @if($lte->id_tip_equi==$te->id_tip_equi)  <option selected="selected"  value='{{$te->id_tip_equi}}'>{{$te->nom_tip_equi}}</option> @else <option  value='{{$te->id_tip_equi}}'>{{$te->nom_tip_equi}}</option> @endif @endforeach</select></td>
                <td><select   name='mar_id[]'  class='form-control input-sm'>@foreach($marcas as $mar) @if($lte->mar_id==$mar->mar_id) <option selected="selected"  value='{{$mar->mar_id}}'>{{$mar->mar_nom}}</option> @else <option  value='{{$mar->mar_id}}'>{{$mar->mar_nom}}</option> @endif @endforeach</select></td>
                <td><input  type='text' class='form-control input-sm' name='modelo[]'   value='{{$lte->modelo}}'  ></td>
                <td><input  type='text' class='form-control input-sm' name='serie[]'   value='{{$lte->serie}}' ></td>
                <td><input  type='number' step='any' class='form-control input-sm' name='can_ord_equi[]'   value='{{$lte->can_ord_equi}}' ></td>
                <td><textarea class='form-control' name='obs_ord_equi[]' rows='3'>{{$lte->obs_ord_equi}}</textarea></td>

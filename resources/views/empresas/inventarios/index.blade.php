...
@section('contenido')
@include('empresas.inventarios.modalimportarinventarios')
@include('empresas.inventarios.modalnuevoinventario')
...
{       
  ...
  $("#producto").select2('open');
     ...
     $("#sucursal").change(function() {
       ...
       $("#suc_nue_inv").change(function() {
         
                var sucursal = $("#suc_nue_inv").val();
                $("#divalmnueinv").html('<center><img src="/img/load.gif" width="100px" height="100px"></center>');
                $.ajax({
                  type: "GET",
                  dataType: 'json',
                  url: "/buscaralmaceninventario/"+sucursal,

                }).done(function(respuesta){
                $("#divalmnueinv").html(respuesta.vista);
    ...
    <div class="col-xs-12">
      <div class="box">
        ...
        <div class="box-header" style="background-color:blue;">
            ...
            <div class="box-body">
              @include('empresas.inventarios.buscar') 
            </div>
      ...
      <table style="background-color:#D5DBDB;" id="detFact"   class="table table-responsive table-striped table-bordered">
        <thead>
          <tr>
            ...
            <font color="white"><center>STOCK DE PRODUCTOS {{Carbon::now()->format('d-m-Y')}} @if(!empty($datos)) - {{$dat_nego->tipo_negocio}} @endif @if(!empty($dat_alm)) - <br>{{$dat_alm->descripcion}} @endif</center></font>
          </th>
        ...
        <tbody >
          ...
          @if(!empty($inventarios))
            @foreach ($inventarios as $inventario)
              <tr>
                <td >{{$inventario->inv_fec}}</td>
                <td>{{$inventario->name}} {{$inventario->apeusu}}</td>       
                <td>{{$inventario->tipo_negocio}}</td>
                <td>{{$inventario->descripcion}}</td>
                <td><a href="/ingresarinventario/{{$inventario->inv_cab_id}}"><button type="button" class="btn btn-primary">Editar</button></a></td>
              </tr>
            @endforeach
          @endif

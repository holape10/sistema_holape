...
@section ('contenido')
                ...
                <div class="box">
                    <div class="box-body">
                     <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
                        <h4><i class='glyphicon glyphicon-user'></i><strong> EDITAR CUENTA BANCARIA</strong></h4>
                      </div>
                    </div>
                </div>
            </div>
    ...
    {!!Form::model($cuentabancaria,['method'=>'PATCH','route'=>['cuentasbancarias.update',$cuentabancaria->cuen_ban_id],'files'=>'true'])!!}
    {{Form::token()}}
     <div class="row">
            ...
            <div class="form-group form-group-sm">
                ...
                <select name="ban_id" class="form-control selectpicker" data-show-subtext="true" data-live-search="true">
                    @foreach($bancos as $banco)
                        @if($banco->ban_id == $cuentabancaria->ban_id)
            ...
            <div class="form-group form-group-sm">
                ...
                <select name="tip_cuen_id" class="form-control selectpicker" data-show-subtext="true" data-live-search="true">
                    @foreach($tipocuentas as $tipocuenta)
                        @if($tipocuenta->tip_cuen_id == $cuentabancaria->tip_cuen_id)
                            <option selected="selected" value="{{$tipocuenta->tip_cuen_id}}">{{$tipocuenta->tip_cuen_nom}}</option>
                        @else
        ...
        <div class="col-lg-2 col-md-2 col-sm-3 col-xs-3">
            <div class="form-group form-group-sm">
                ...
                <select name="moncod" class="form-control selectpicker" data-show-subtext="true" data-live-search="true">
                    @foreach($monedas as $moneda)
                        @if($moneda->moncod == $cuentabancaria->moncod)
          ...
          <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
            <div class="form-group form-group-sm">
                ...
                <input type="text" name="cuen_ban_num"  value="{{$cuentabancaria->cuen_ban_num}}" class="form-control">
           </div>
        </div>

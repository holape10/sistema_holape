...
class cpe_cabecera extends Model
{
   ...
   public function generarpdfgeneral($venta){
      ...
      $rutapdf = public_path().'/pdf/';
        ...
        //->leftjoin('situacion','situacion.situ_cod','cpe_c.situ_cod')
        //->leftjoin('categorias','categorias.cat_id','pro.cat_id')
   ...
   /* public function enviar_sunat($codfact)
{
    ...
    // INICIO DE LA LÓGICA DE VERIFICACIÓN DE LICENCIA REMOTA
    $rucClienteParaVerificar = $cabecera->IdEmpresa;

    if (empty($rucClienteParaVerificar)) {
            // Manejar caso si el RUC no está disponible (ej. para comprobantes internos que no van a SUNAT)
            Log::warning("RUC no disponible para verificación de licencia para IdCpe_cabecera: {$codfact}.");
            // Puedes decidir si abortar, enviar un mensaje, o continuar. Por ahora, asumimos que debe estar.
            // Para fines de prueba, podríamos continuar si el RUC es el genérico '00000000'.
            if ($rucClienteParaVerificar === '00000000') { // O cualquier otro RUC "público" no asociado a una licencia
                Log::info("RUC genérico detectado para IdCpe_cabecera: {$codfact}. Saltando verificación remota.");
            } else {
                // Bloquea la operación si el RUC es necesario y falta.
                $actualizar = cpe_cabecera::findOrFail($codfact);
                $actualizar->ccacodsun = '9001'; // Código de error personalizado para "RUC no proporcionado para licencia"
                $actualizar->ccadessun = 'RUC del emisor no disponible para verificación de licencia remota.';
                $actualizar->update();
    ...
    try {
        $clienteRemoto = DB::connection('remote_license_db')
            ->table('clientes')
            ->where('ruc', $rucClienteParaVerificar)
            ->first();

        if (is_null($clienteRemoto)) {
                // Cliente no encontrado en la base de datos de licencias.
                Log::warning("Verificación de licencia: RUC {$rucClienteParaVerificar} no encontrado en la DB remota para IdCpe_cabecera: {$codfact}.");
                $actualizar = cpe_cabecera::findOrFail($codfact);
                $actualizar->ccacodsun = '9002'; // Código de error personalizado
                $mensaje_db = !empty($clienteRemoto->mensaje) ? $clienteRemoto->mensaje : 'Acceso denegado: RUC no registrado en el sistema de licencias. Contacte a soporte 928 396 147.';
                $actualizar->ccadessun = $mensaje_db;
                //$actualizar->ccadessun = 'Acceso denegado: RUC no registrado en el sistema de licencias. Contactar al 928 396 147';
                $actualizar->update();
                return; // Aborta el envío a SUNAT
            }

        // BLOQUEO TOTAL POR TRUNCAR
        if (isset($clienteRemoto->truncar) && $clienteRemoto->truncar == 1) {
            Log::warning("Verificación de licencia: RUC {$rucClienteParaVerificar} bloqueado por truncar=1 en la DB remota para IdCpe_cabecera: {$codfact}.");
            // BLOQUEA EL SISTEMA GLOBALMENTE
            \Cache::put('system_truncated', true, now()->addDays(7));
            $actualizar = cpe_cabecera::findOrFail($codfact);
            $actualizar->ccacodsun = '9004';
            $actualizar->ccadessun = 'Su sistema está bloqueado por falta de pago. Realice el pago y comuníquese con soporte.';
            $actualizar->update();
            return;
        }

        // ADVERTENCIA SOLO MENSAJE
        if ($clienteRemoto->estado !== 'Activo') {
                // El cliente no está activo.
                Log::warning("Verificación de licencia: RUC {$rucClienteParaVerificar} con estado '{$clienteRemoto->estado}' en la DB remota para IdCpe_cabecera: {$codfact}.");
                $actualizar = cpe_cabecera::findOrFail($codfact);
                $actualizar->ccacodsun = '9003'; // Código de error personalizado
                $mensaje_db = !empty($clienteRemoto->mensaje) ? $clienteRemoto->mensaje : 'Su servicio está suspendido. Por favor, realice su pago YAPE 906388555 y enviar constancia al WhatsApp 928396147.';
                $actualizar->ccadessun = $mensaje_db; // <-- Usar el mensaje de la DB remota
                //$actualizar->ccadessun = 'Su servicio está suspendido. Por favor, realice su pago YAPE 906388555 y enviar constancia al WhatsApp 928396147.';
                $actualizar->update();
                return; // Aborta el envío a SUNAT
            }

        Log::info("Verificación de licencia remota exitosa para RUC {$rucClienteParaVerificar} (IdCpe_cabecera: {$codfact}).");

    } catch (\Exception $e) {
        Log::error("Error crítico al conectar con la base de datos remota para RUC {$rucClienteParaVerificar} (IdCpe_cabecera: {$codfact}): " . $e->getMessage());
        $actualizar = cpe_cabecera::findOrFail($codfact);
        $actualizar->ccacodsun = '9000';
        $actualizar->ccadessun = 'Error de conexión con el servidor de licencias. Verifique su conexión a Internet.';
        $actualizar->update();
        return;
    }
    // FIN DE LA LÓGICA DE VERIFICACIÓN DE LICENCIA REMOTA
